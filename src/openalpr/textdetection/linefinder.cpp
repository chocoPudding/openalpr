...
std::vector<cv::Point> LineFinder::findNextBestLine(cv::Size imageSize, std::vector<cv::Point> bestLine)
{
    ...
    for (unsigned int i = 0; i < pipeline_data->thresholds.size(); i++)
    {
        ...
        for (unsigned int hitidx = 0; hitidx < histogram_hits.size(); hitidx++)
        {
            ...
            if ((hit.first >= transformed_best_line_start - transformed_best_line_variance) && (hit.first <= transformed_best_line_start + transformed_best_line_variance) && (hit.second >= transformed_best_line_end - transformed_best_line_variance) && (hit.second <= transformed_best_line_end + transformed_best_line_variance))
            {
        ...
        for (unsigned int hitidx = 0; hitidx < histogram_hits.size(); hitidx++)
        {
            ...
            if ((hit_width >= ideal_width * (1 - max_deviation_percent)) && (hit_width <= ideal_width * (1 + max_deviation_percent)))
            {
...
vector<Point> LineFinder::getBestLine(const TextContours contours, vector<CharPointInfo> charPoints)
{
    ...
    for (unsigned int i = 0; i < charPoints.size() - 1; i++)
    {
        for (unsigned int k = i + 1; k < charPoints.size(); k++)
        {
            ...
            if (abs(top.angle) <= pipeline_data->config->maxPlateAngleDegrees && abs(parallelBot.angle) <= pipeline_data->config->maxPlateAngleDegrees)
            {
            ...
            // Only allow lines that have a sane angle
            if (abs(parallelTop.angle) <= pipeline_data->config->maxPlateAngleDegrees && abs(bottom.angle) <= pipeline_data->config->maxPlateAngleDegrees)
            {
    ...
    for (unsigned int i = 0; i < topLines.size(); i++)
    {
        ...
        for (unsigned int charidx = 0; charidx < charPoints.size(); charidx++)
        {
            ...
            if ((topYPos >= minTop && topYPos <= maxTop) && (botYPos >= minBot && botYPos <= maxBot))
            {
                curScore++;
            }
        }
        ...
        if ((curScore > bestScore) || (curScore == bestScore && topLines[i].length > bestScoreDistance))
        {
            bestScore = curScore;
            bestScoreIndex = i;
