...
Detector::Detector(Config* config, PreWarp* prewarp)
    : detector_mask(config, prewarp)
{
    this->config = config;
...
vector<PlateRegion> Detector::detect(Mat frame, std::vector<cv::Rect> regionsOfInterest)
{
    ...
    for (int i = 0; i < regionsOfInterest.size(); i++)
    {
        ...
        // then skip it
        if ((roi.width < config->minPlateSizeWidthPx) || (roi.height < config->minPlateSizeHeightPx))
            continue;
