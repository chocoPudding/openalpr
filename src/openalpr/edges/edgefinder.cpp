...
vector<cv::Point> EdgeFinder::highContrastDetection(Mat newCrop, vector<TextLine> newLines)
{
    ...
    for (unsigned int i = 0; i < contours.size(); i++)
    {
        ...
        float polygon_width = (distanceBetweenPoints(sorted_polygon_points[0], sorted_polygon_points[1]) + distanceBetweenPoints(sorted_polygon_points[3], sorted_polygon_points[2])) / 2;
        float polygon_height = (distanceBetweenPoints(sorted_polygon_points[2], sorted_polygon_points[1]) + distanceBetweenPoints(sorted_polygon_points[3], sorted_polygon_points[0])) / 2;
        ...
        if (rrect.center.x < (newCrop.cols * MAX_CLOSENESS_TO_EDGE_PERCENT) || rrect.center.x > (newCrop.cols - (newCrop.cols * MAX_CLOSENESS_TO_EDGE_PERCENT)) || rrect.center.y < (newCrop.rows * MAX_CLOSENESS_TO_EDGE_PERCENT) || rrect.center.y > (newCrop.rows - (newCrop.rows * MAX_CLOSENESS_TO_EDGE_PERCENT)))
        {
            continue;
        }
