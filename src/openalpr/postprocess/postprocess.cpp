...
void PostProcess::insertLetter(string letter, int line_index, int charposition, float score)
{
    ...
    {
        if (letters[charposition][i].letter == letter && letters[charposition][i].line_index == line_index && letters[charposition][i].charposition == charposition)
        {
            existingIndex = i;
            break;
        }
...
bool PostProcess::analyzePermutation(vector<int> letterIndices, string templateregion, int topn)
{
    ...
    if (plate_char_length < config->postProcessMinCharacters || plate_char_length > config->postProcessMaxCharacters)
        return false;
    ...
    if (!config->mustMatchPattern || templateregion.size() == 0 || (config->mustMatchPattern && possibility.matchesTemplate))
    {
