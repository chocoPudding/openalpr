...
struct Frag
{
    ...
    Frag(uint32 begin, PatchList end)
        : begin(begin)
        , end(end)
    {
    }
...
Frag Compiler::Cat(Frag a, Frag b)
{
    ...
    if (begin->opcode() == kInstNop && a.end.p == (a.begin << 1) && begin->out() == 0)
    {
...
int Compiler::RuneByteSuffix(uint8 lo, uint8 hi, bool foldcase, int next)
{
    ...
    if (encoding_ == kEncodingLatin1 || (encoding_ == kEncodingUTF8 && !reversed_ && !(0x80 <= lo && hi <= 0xbf)))
    {
        return UncachedRuneByteSuffix(lo, hi, foldcase, next);
    }

    uint64 key = (uint64)next << 17 | (uint64)lo << 9 | (uint64)hi << 1 | (uint64)foldcase;
