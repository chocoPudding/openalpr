    ...
    , cache_warned_(false)
{
    ...
    int64 one_state = sizeof(State) + (prog_->size() + nmark) * sizeof(int) + (prog_->bytemap_range() + 1) * sizeof(State*);
...
DFA::State* DFA::WorkqToCachedState(Workq* q, uint flag)
{
    ...
    for (Workq::iterator it = q->begin(); it != q->end(); ++it)
    {
        ...
        case kInstAltMatch:
            ...
            if (kind_ != Prog::kManyMatch && (kind_ != Prog::kFirstMatch || (it == q->begin() && ip->greedy(prog_))) && (kind_ != Prog::kLongestMatch || !sawmark) && (flag & kFlagMatch))
            {
                delete[] inst;
                if (DebugDFA)
...
void DFA::AddToQueue(Workq* q, int id, uint flag)
{
    ...
    while (nstk > 0)
    {
        ...
        case kInstAltMatch:
            ...
            stk[nstk++] = ip->out1();
            if (q->maxmark() > 0 && id == prog_->start_unanchored() && id != prog_->start())
                stk[nstk++] = Mark;
            stk[nstk++] = ip->out();
            break;
    ...
    while (p != ep)
    {
        ...
        if (ns == NULL)
        {
            ...
            if (ns == NULL)
            {
                ...
                if (FLAGS_re2_dfa_bail_when_slow && resetp != NULL && static_cast<unsigned long>(p - resetp) < 10 * state_cache_.size())
                {
                    params->failed = true;
                    return false;
                }
...
bool DFA::FastSearchLoop(SearchParams* params)
{
    ...
    int index = 4 * have_firstbyte + 2 * params->want_earliest_match + 1 * params->run_forward;
