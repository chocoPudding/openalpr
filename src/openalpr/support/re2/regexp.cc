...
static bool TopEqual(Regexp* a, Regexp* b)
{
    ...
    case kRegexpLiteralString:
        return a->nrunes() == b->nrunes() && ((a->parse_flags() ^ b->parse_flags()) & Regexp::FoldCase) == 0 && memcmp(a->runes(), b->runes(), a->nrunes() * sizeof a->runes()[0]) == 0;
    ...
    case kRegexpCharClass:
    {
        CharClass* acc = a->cc();
        CharClass* bcc = b->cc();
        return acc->size() == bcc->size() && acc->end() - acc->begin() == bcc->end() - bcc->begin() && memcmp(acc->begin(), bcc->begin(), (acc->end() - acc->begin()) * sizeof acc->begin()[0]) == 0;
    }
    }
