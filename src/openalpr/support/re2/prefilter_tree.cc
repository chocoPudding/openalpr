...
static bool KeepPart(Prefilter* prefilter, int level)
{
    ...
    case Prefilter::ATOM:
        return prefilter->atom().size() >= static_cast<size_t>(FLAGS_filtered_re2_min_atom_len);
    ...
    for (size_t i = 0; i < entries_.size(); i++)
    {
        ...
        if (parents->size() > 8)
        {
            ...
            for (StdIntMap::iterator it = parents->begin();
                 it != parents->end(); ++it)
            {
                have_other_guard = have_other_guard && (entries_[it->first].propagate_up_at_count > 1);
            }
...
void PrefilterTree::AssignUniqueIds(vector<string>* atom_vec)
{
    ...
    for (int i = v.size() - 1; i >= 0; i--)
    {
        ...
        case Prefilter::AND:
        {
            ...
            entry->propagate_up_at_count = prefilter->op() == Prefilter::AND ? uniq_child.size() : 1;
