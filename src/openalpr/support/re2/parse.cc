...
static void AddFoldedRange(CharClassBuilder* cc, Rune lo, Rune hi, int depth)
{
    ...
    while (lo <= hi)
    {
        ...
        if (f == NULL) // lo has no fold, nor does anything above lo
            break;
        if (lo < f->lo)
        { // lo has no fold; next rune with a fold is f->lo
            lo = f->lo;
            continue;
        }
...
bool Regexp::ParseState::DoVerticalBar()
{
    ...
    Regexp* r1;
    Regexp* r2;
    if ((r1 = stacktop_) != NULL && (r2 = stacktop_->down_) != NULL && r2->op() == kVerticalBar)
    {
        // If above and below vertical bar are literal or char class,
        // can merge into a single char class.
        Regexp* r3;
        if ((r1->op() == kRegexpLiteral || r1->op() == kRegexpCharClass || r1->op() == kRegexpAnyChar) && (r3 = r2->down_) != NULL)
        {
            Rune rune;
            switch (r3->op())
            {
    ...
    int maxdepth)
{
    ...
    for (int i = 0; i <= n; i++)
    {
        ...
        if (i < n && (sub[i]->op() == kRegexpLiteral || sub[i]->op() == kRegexpCharClass))
            continue;
    ...
    for (int i = 0; i < n; i++)
    {
        if (i + 1 < n && sub[i]->op() == kRegexpEmptyMatch && sub[i + 1]->op() == kRegexpEmptyMatch)
        {
            sub[i]->Decref();
            continue;
        }
    ...
    RegexpStatus* status)
{
    ...
    while (t.size() > 0)
    {
        ...
        case '\\':
        { // Escaped character or Perl sequence.
