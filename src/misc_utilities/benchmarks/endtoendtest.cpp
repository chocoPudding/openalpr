...
void EndToEndTest::runTest(string country, vector<std::string> files)
{
    ...
    for (int i = 0; i < benchmarkResults.size(); i++)
    {
        if (benchmarkResults[i].detectedPlate)
            totalDetections++;
        if (benchmarkResults[i].topResultCorrect)
            totalTopResultCorrect++;
        if (benchmarkResults[i].top10ResultCorrect)
            totalTop10Correct++;
...
bool EndToEndTest::rectMatches(cv::Rect actualPlate, PlateRegion candidate)
{
    ...
    if (hasAllPoints && (sizeDiff < MAX_SIZE_PERCENT_LARGER))
